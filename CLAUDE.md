...
>
> Next up: **Stage 7 — Polish & testing** (theme selector, PWA + QR-scan testing on real Android & iOS, anti-pause GitHub Action). Before or during Stage 7, revisit "Known gaps and deferred decisions" below — several of its IMPORTANT items (missing tests on `join-list.ts`/`list-detail.ts`, the `ConfirmModal`-for-non-destructive-action inconsistency, the live-update gaps) are natural candidates to fold into that stage's polish work.
>
...
- `ConfirmModal` (styled `btn-danger`, `role="alertdialog"`) is used for "Regenerate invitation code" in `list-invite.html`, a non-destructive action (it doesn't destroy data — old code just stops working for new requests). This is the one place in the app where the danger-styled confirmation pattern is applied to a non-destructive action. Either justify/document it as an intentional exception (as was done for reusing `ConfirmModal`'s CSS in `ChooseSuccessorDialog`), or switch it to a neutral-styled confirmation.
