...
>
> Next up: rest of **Stage 7 — Polish & testing** (PWA + QR-scan testing on real Android & iOS, anti-pause GitHub Action). Before or during it, revisit "Known gaps and deferred decisions" below — its remaining IMPORTANT items (missing full-coverage tests on `list-detail.ts`'s state machine, the live-update gaps) are natural candidates to fold into that stage's polish work.
>
...
- `list-detail.ts` (the leave-list branch routing) has no full-coverage unit tests for its state machine — it only gained targeted tests for the TOCTOU fix in the post-Stage-6 audit. `confirm-modal.ts`'s focus-trap behavior is now covered indirectly via `focus-trap.directive.spec.ts`. (`join-list.ts` — previously the other item on this line — gained full test coverage in Stage 7; see "Current stage".)
