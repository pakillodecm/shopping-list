...
>
> - Added full unit test coverage for `join-list.ts` (RF-15, the QR-scan camera state machine, previously the most complex untested logic in the project — see `join-list.spec.ts`). jsdom has no real camera/`getUserMedia`/wasm decoder, so `NgxScannerQrcodeComponent` is stood in for via `TestBed.overrideComponent` with a minimal fake exposing only what `join-list.ts` actually calls on it (`devices`, `stop()`, `playDevice()`, `loadCameraDevices()`, the `event` output) — the real viewChild+effect wiring runs unmodified against that fake. Covers: the full `QrState` machine, all three `navigator.permissions` outcomes plus the Safari/no-API and query-throws degradations (CA-13.7), scan → lookup → confirm → request (including invalid-code and already-pending), back-camera selection/enumeration, the manual code tab, and mode-switch cleanup. Real camera/hardware behavior stays manually verified on-device (Stage 5 and the rest of Stage 7).
> - Added complementary unit test coverage for `list-detail.ts`'s leave-list state machine (`list-detail.spec.ts`), beyond the TOCTOU-specific tests already there: `startLeaveList`'s three branches plus its `getListMembers`-failure path, `confirmSuccessorChoice`, `leaveConfirmMessage`'s per-branch copy, `toReadableLeaveError`'s message mapping, and the `subscribeToList` Realtime handler (list rename/delete, reconnect refetch success/failure).
> - Added `.github/workflows/keep-supabase-awake.yml` (the anti-pause action, "Availability" in the source of truth's Tech stack section): a scheduled (Mondays and Thursdays) plus manually-dispatchable workflow that calls the `get_list_name_by_code` RPC with the anon key and a code the invitation-code alphabet can never generate (`"000000"`, since `0` is an excluded ambiguous character), forcing a real read against Postgres without authentication or risk of leaking a real list's name. Fails the job (non-2xx from `curl --fail-with-body`) if the request doesn't succeed, so a broken ping is visible in the Actions tab instead of silently no-op'ing.
> - Fixed a real data-consistency bug found during real-device testing: if a user was viewing `/lists` and got expelled from a list (`remove_member`) — or, it turns out, left one themselves (`leave_list`) from a second device — the list didn't disappear until reload. Root cause was a genuine Supabase Realtime platform limitation, not an app bug per se (see the new "Key domain reminders" entry): a `memberships` DELETE payload can never carry `list_id`/`user_id`, only its own primary key, and can't be server-side filtered at all. `subscribeToMyMemberships`/`subscribeToListMembers` (`list.service.ts`) and their three call sites (`lists.ts`'s membership subscription, `list-members.ts`'s own-membership and roster subscriptions) now treat any DELETE as a "resync" signal (reusing the same full-refetch each already had for reconnect) instead of trying to remove a specific row by an id the payload never actually contains.
> - Fixed (pending on-device confirmation) the iPad back-camera bug found during real-device testing: the QR scanner opened the front (wide-angle) camera by default on iPad even though a rear camera was available. Investigated what iPad Safari/WebKit actually exposes for camera selection before touching anything: `device.label` from `enumerateDevices()` has no documented, reliable orientation keyword on WebKit at all ("there's no identifier whether a device is a front or back camera" — confirmed against public W3C mediacapture-main / WebKit bug discussions), so the existing `/back|rear|environment/i` label regex was never guaranteed to match iPad's actual labels — expanding it further would have been guessing. `startScanning()` in `join-list.ts` now requests the initial camera with `getUserMedia({video: {facingMode: {ideal: 'environment'}}}})` and reads back which physical camera got resolved via `track.getSettings().deviceId`, trusting that over the label regex whenever it matches one of the enumerated devices (falling back to the regex otherwise) — the same technique other production QR-scanning libraries (e.g. `html5-qrcode`) use for this exact cross-browser problem, and it doesn't change behavior on Android/iPhone (already correctly handled, and now handled the same way, just via a more reliable signal). Documented as NOT provably 100% reliable on every iPad model/iOS version — WebKit has known, cited regressions even in the `facingMode` constraint itself (e.g. WebKit bug #253186) — so the existing manual "switch camera" dropdown remains the deliberate user-facing recovery path if a specific device still guesses wrong.
> - Fixed (pending on-device confirmation) the QR-scanner loading-transition polish flagged by real-device testing: the black video box (`.qr-video-wrap`, always black by design) showed with no content and a plain muted status line below it while the camera was still starting, reading as a half-broken layout rather than a deliberate loading state. Added a `.qr-loading-overlay` (spinner + "Preparando la cámara…", `role="status"`) covering the video box during the `'starting'` `QrState` only, styled with fixed light-on-black colors (not theme tokens, matching the video box's own always-black background in both themes) plus `--color-primary` for the spinner accent; respects `prefers-reduced-motion`. Purely visual — no state machine or navigation changes.
>
> Next up: rest of **Stage 7 — Polish & testing** (PWA + QR-scan testing on real Android & iOS, confirming the two fixes above on real hardware). Before or during it, revisit "Known gaps and deferred decisions" below — its remaining IMPORTANT items (the live-update gaps) are natural candidates to fold into that stage's polish work.
>
...
- `confirm-modal.ts`'s focus-trap behavior is now covered indirectly via `focus-trap.directive.spec.ts`. (`join-list.ts` and `list-detail.ts`'s leave-list state machine — previously the other items on this line — both gained full test coverage in Stage 7; see "Current stage".)
