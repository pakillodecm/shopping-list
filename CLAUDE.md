...
>
> Next up: rest of **Stage 7 — Polish & testing** (theme selector, PWA + QR-scan testing on real Android & iOS, anti-pause GitHub Action). Before or during it, revisit "Known gaps and deferred decisions" below — several of its remaining IMPORTANT items (missing tests on `join-list.ts`/`list-detail.ts`, the live-update gaps) are natural candidates to fold into that stage's polish work.
>
